...
//Returns a String that is the content of a Scene File that makes a Spiral Staircase with 50 steps
String App::makeStairs() {
    String name = "{\n name = staircase;\n";

    String models = "models = {\n";
    models += "stair = ArticulatedModel::Specification {\n";
    models += "filename = \"model/crate/crate.obj\";\n";
    models += "preprocess = {\n";
    models += "setMaterial(all(), \"wood-texture.jpg\");\n";
    models += "transformGeometry(all(), Matrix4::scale(0.5, 0.2, 1.0));\n";
    models += "transformGeometry(all(), Matrix4::translation(0, 0, 1.0));\n";
    models += "};\n};\n};\n";
    ...
    for(int i = 0; i < 50; i++) {
        int yaw = (i * 20) % 360;
        entities += "stair" + (String) std::to_string(i) + " = VisibleEntity {\n";
        entities += "model = \"stair\";\n";
        entities += "frame = CFrame::fromXYZYPRDegrees(0, " + (String) std::to_string(i*0.2) + ",0 ," + (String) std::to_string(yaw) + ", 0, 0);\n};\n";
    }

    entities += "camera = Camera {\n";
    entities += "frame = CFrame::fromXYZYPRDegrees(0, 0, 5);\n};\n};\n};\n";

    return name + models + entities;
}

//Generates my staircase Scene file when called in the filename passed
void App::createScene(String sceneName) {
    TextOutput output = TextOutput("scene\\" + sceneName + ".Scene.Any");

    String staircase = makeStairs();
    output.writeSymbols(staircase);
